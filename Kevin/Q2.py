...
def carregar_imagens(dim=50, pasta="RecFac", classes=20):
    X, Y = [], []
    for i, p in enumerate(os.listdir(pasta)):
        for img in os.listdir(os.path.join(pasta, p)):
            img_resized = cv2.resize(cv2.imread(os.path.join(pasta, p, img), cv2.IMREAD_GRAYSCALE), (dim, dim))
            X.append(img_resized.flatten() / 255.0)
            y = np.full(classes, -1); y[i] = 1; Y.append(y)
    X, Y = np.array(X).T, np.array(Y).T
    return (X - X.mean(axis=1, keepdims=True)) / (X.std(axis=1, keepdims=True) + 1e-8), Y

class RNA:
    def __init__(self, inp, out, lr=0.001, epochs=50): 
        self.w, self.b, self.lr, self.ep = np.random.randn(out, inp) * 0.01, np.zeros((out, 1)), lr, epochs
    ...
    def prever(self, X): 
        ...
        Y = -np.ones(z.shape); idx = np.argmax(z, axis=0)
        for i, ix in enumerate(idx): Y[ix, i] = 1
        return Y
...
def calcular_metricas(Y_true, Y_pred):
    ...
    return (tp.sum() + tn.sum()) / max(tp.sum() + tn.sum() + fp.sum() + fn.sum(), 1), tp.sum() / max(tp.sum() + fn.sum(), 1), tn.sum() / max(tn.sum() + fp.sum(), 1)
...
def matriz_confusao(Y_true, Y_pred, classes):
    ...
    for t, p in zip(true_labels, pred_labels): cm[t, p] += 1
    return cm
...
def plot_matriz_confusao(cm, title="Matriz de Confusão"):
    ...
    for i in range(len(cm)):
        for j in range(len(cm)):
            plt.text(j, i, f"{cm[i, j]}", ha="center", va="center", color="white" if cm[i, j] > cm.max() / 2 else "black", fontsize=12)
    plt.tight_layout()
    plt.show()
...
def monte_carlo(modelo, X, Y, rodadas=50):
    ...
    for _ in tqdm(range(rodadas), desc="Monte Carlo"):
        ...
        Xt, Yt, Xv, Yv = X[:, idx[:split]], Y[:, idx[:split]], X[:, idx[split:]], Y[:, idx[split:]]
    ...
    return np.mean(resultados, axis=0), np.mean(confusoes, axis=0)
